    ...
    <div class="mx-auto max-w-2xl space-y-6">
        <div class="flex items-center gap-4">
            <x-ui.button
                variant="ghost"
                size="icon"
                href="{{ route('admin.service-departures.index') }}"
                class="rounded-full"
            >
            ...
            <h2 class="text-2xl font-bold tracking-tight">
                {{ isset($departure) ? 'Edit Departure' : 'Add New Departure' }}
            </h2>
        </div>

        <form
            action="{{ isset($departure) ? route('admin.service-departures.update', $departure) : route('admin.service-departures.store') }}"
            method="POST"
        >
            @csrf
            @if (isset($departure))
            ...
            <x-ui.card>
                    ...
                    <x-ui.field>
                        ...
                        >
                            ...
                            @foreach ($services as $service)
                                <option
                                    value="{{ $service->id }}"
                                    @selected(old('service_id', $departure->service_id ?? '') == $service->id)
                                >
                                    {{ $service->title }}
                                </option>
                            @endforeach
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <x-ui.field>
                            ...
                            <x-ui.input
                                type="date"
                                name="start_date"
                                value="{{ old('start_date', isset($departure) ? $departure->start_date->format('Y-m-d') : '') }}"
                                required
                            />
                        ...
                        <x-ui.field>
                            ...
                            <x-ui.input
                                type="date"
                                name="end_date"
                                value="{{ old('end_date', isset($departure) ? $departure->end_date->format('Y-m-d') : '') }}"
                                required
                            />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <x-ui.field>
                            ...
                            <x-ui.input
                                ...
                                min="0"
                                name="seats_booked"
                                value="{{ old('seats_booked', $departure->seats_booked ?? '0') }}"
                            />
                    ...
                    <x-ui.field>
                        ...
                        >
                            @foreach (['Available', 'Guaranteed', 'Full', 'Cancelled'] as $status)
                                <option
                                    value="{{ $status }}"
                                    @selected(old('status', $departure->status ?? 'Available') == $status)
                                >
                                    {{ $status }}
                                </option>
                            @endforeach
                ...
                <x-ui.card-footer class="flex justify-end gap-2 border-t pt-4">
                    <x-ui.button variant="outline" href="{{ route('admin.service-departures.index') }}">
                        Cancel</x-ui.button>
