...
<button type="button" x-ref="trigger" x-init="_trigger = $el"
    @if ($ariaLabel) aria-label="{{ $ariaLabel }}" @endif @click="toggleList()"
    @keydown.down.prevent.stop="openList()" @keydown.up.prevent.stop="openList()" @keydown.enter.prevent.stop="openList()"
    @keydown.space.prevent.stop="openList()" role="combobox" aria-haspopup="listbox" :aria-controls="$id('blat-listbox')"
    :aria-expanded="open" :data-state="open ? 'open' : 'closed'" :data-placeholder="label ? null : true"
