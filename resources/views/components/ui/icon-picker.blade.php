...
@php
    ...
    if ($currentValue && preg_match('/^[a-z0-9\-]+$/', $currentValue)) {
        ...
        if (file_exists($path)) $selectedSvg = file_get_contents($path);
    }
@endphp
...
>
    ...
    >
        ...
        <svg xmlns="http://www.w3.org/2000/svg" class="ml-auto size-4 shrink-0 text-muted-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
    </button>
    ...
    >
        ...
        <div class="border-b p-2">
            <input
                ...
                class="border-input placeholder:text-muted-foreground flex h-8 w-full rounded-md border bg-transparent px-2 text-sm outline-none focus:border-ring"
        ...
        <div class="border-b px-2 py-1">
            <button type="button" @click="clear()" class="text-muted-foreground hover:text-foreground w-full rounded px-2 py-1 text-left text-xs">
                — No icon
            </button>
        </div>
        ...
        <div
            ...
            @scroll="$event.target.scrollTop + $event.target.clientHeight >= $event.target.scrollHeight - 40 && loadMore()"
        >
            {{-- Loading state --}}
            <template x-if="loading">
                <div class="col-span-8 flex items-center justify-center py-6 text-muted-foreground text-sm gap-2">
                ...
                <template x-for="icon in visible" :key="icon">
                    ...
                    >
                        <span
                            ...
                            x-html="svgCache[icon] || '<svg xmlns=\'http://www.w3.org/2000/svg\' class=\'size-4 opacity-20\' viewBox=\'0 0 24 24\' fill=\'none\' stroke=\'currentColor\' stroke-width=\'2\'><rect width=\'18\' height=\'18\' x=\'3\' y=\'3\' rx=\'2\'/></svg>'"
                        ></span>
                    </button>
                </template>
            </template>

            <p x-show="!loading && filtered.length === 0" class="text-muted-foreground col-span-8 py-4 text-center text-sm">No icons found.</p>
        </div>
    </div>
</div>
